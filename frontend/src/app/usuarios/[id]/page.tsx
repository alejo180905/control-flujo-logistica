...
export default function EditarUsuarioPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchUsuario = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/usuarios/${params.id}`, {
          headers: {
        ...
        setError(`Error al cargar el usuario: ${error?.message || 'Error desconocido'}`)
      } finally {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      const token = document.cookie.split(';').find(c => c.trim().startsWith('token='))?.split('=')[1]
              const response = await fetch(`http://localhost:3000/api/usuarios/${params.id}/toggle-status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        }
